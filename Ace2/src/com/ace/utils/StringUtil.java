...
public class StringUtil {
    ...
    /**
     * 설명 : 해당문자열을 카드번호 ( 9999-xxxx-xxxx-9999 ) 변환한다.
     * @param str 원본 사업자번호
     * @return String
     */
    public static String formatCardNo(String str){
    	if ( str != null && str.length()==16 ) return str.substring(0, 4)+" **** **** "+str.substring(str.length()-4, str.length());
    	else	return	str; 
    }
    /**
    ...
    public static int nullToPageSize(String str){
...
/**
    ...
    /**
     * <p>
     * 문자열이 지정한 길이와 맞는지 확인하는 메서드
     * </p>
     * @param source 문자열
     * @param slength 지정길이
     * @return 확인여부
     */
    public static boolean compareLength(String source, int slength) 
    {
    ...
    public static boolean isAlphabet(String val)
    {
        ...
        {
            if ( !regex.contains(val.subSequence(i, i+1)) )
            {
                output = false;
                break;
            }
    ...
    public static boolean isAlphabetNumber(String val)
    {
        String regex = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890";
        boolean output = true;
    ...
    public static String toCardNo(String number) {
    	...
    	for (int idx = 0 ; idx < len ; idx++) {
    		if (idx > 0 && idx % 4 == 0) {
    			list.add(number.substring(beginIdx, idx));
    			beginIdx += 4;
    			if (idx == 12) list.add(number.substring(idx));
    		}
    	}
